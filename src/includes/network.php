...
class HerissonNetwork {
	...
	public function download($url,$post=array()) {
		...
		if($this->http_code >= 400) {
			...
			return new WP_Error('herisson',sprintf(__("The site %s returned a %s error (%s).",HERISSON_TD),$url,$this->http_code,$this->http_message),$this->http_code);
		}

		$content_type = curl_getinfo($curl, CURLINFO_CONTENT_TYPE);
		$result = array("data"=>$content, "type" => $content_type, "code" => $this->http_code);
